...
class Board
  ...
  def mark_number(number)
    ...
    winning_numbers << number if bingo_completed?
  end
...
class Part2
  ...
  def play_bingo
    ...
    bingo_numbers.each do |number|
      ...
      if boards.all?(&:bingo_completed?)
        winning_number = number
        break
      end
    end
    winner_board = boards.find { |b| b.winning_numbers.size == 1 && b.winning_numbers.last == winning_number }
    puts winning_number
